...
repos:
  - name: CP-Tensile-Different-Directions-Dream3D
    ...
    not_in_repo: |
      The PSO-based constitutive parameter calibration pipeline (PySwarms +
      DAMASK integration), the DREAM.3D pipelines for generating the 23-RVE
      carbide variation suite, and the Voce-based empirical model fitting
      scripts are described in the paper and will be published later.
keywords:
  - Crystal plasticity
